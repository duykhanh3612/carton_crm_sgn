  ...
  function formatDate(d, fmt, monthNames, dayNames) {
    ...
    var formatSubSeconds = function (
      ...
      if (numberDecimalPlaces < 6 && numberDecimalPlaces > 0) {
        ...
        totalMicroseconds = Math.round(
          Math.round(totalMicroseconds * magnitude) / magnitude
        );
        formattedString = ("00000" + totalMicroseconds).slice(
          -6,
          -(6 - numberDecimalPlaces)
        );
      } else {
    ...
    plot.hooks.processOptions.push(function (plot) {
      $.each(plot.getAxes(), function (axisName, axis) {
        ...
        if (opts.mode === "time") {
          ...
          if (
            "tickFormatter" in opts &&
            typeof opts.tickFormatter === "function"
          )
            return;

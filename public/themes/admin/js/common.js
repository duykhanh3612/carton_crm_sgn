jQuery(document).ready(function ($) {
    ...
    $.notify = (options) => {
        ...
        options.type =
            options.type &&
                allowTypes.find(
                    (type) => type === options.type.trim().toLowerCase()
                ) ?
                options.type :
                "default";
    ...
    $.changeTab = function (tabs) {
        $(tabs).each(function () {
            if (
                $(this)
                    .data("group")
                    .substr($(this).data("group").length - 1) ==
                $("#current_tab").val()
            ) {
        ...
        $(".update-user-setting").change(function () {
            ...
            if (with_option_text) {
                ele.find("option").each(function (index, item) {
                    if (
                        hsVal === item.value ||
                        ($.isArray(hsVal) && hsVal.indexOf(item.value) >= 0)
                    ) {
            ...
            ele.submitDataAjax({
                ...
                success: function success(res) {
                    if (res.status === 200) {
                        let item = $("#" + data["hs_key"])
                            .parents("td")
                            .find(".map-field-addition-setting");
                        if (res.deleted === true) {
    ...
    //init DataTable
    if($("#example2").hasClass("sortable"))
    {
        table = $("#example2.sortable").DataTable({
            bPaginate: false,
            bInfo: false,
            bFilter: false,
            ...
            aSorting: [parseInt($("#sort_column").val()), $("#sort_order").val()],
        });
        table.on("order.dt", function () {
            var sort = table.order();
            $("#sort_column").val(sort[0][0]);
            $("#sort_field").val(
                $("#example2.sortable tr th")
                    .eq(parseInt(sort[0][0]))
                    .attr("data-field")
            );
            ...
            let currentTab = group ?
                group.substr(
                    $("#full-size-menu li.active a").data("group").length - 1
                ) :
                "";
    ...
    $(".processingModalLink").click(function () {
        if ($(this).data("form")) {
            $("#processingModal #processingModalForm").attr(
                "action",
                $(this).data("href")
            );
            $("#processingModal #processingModalForm").attr(
                "method",
                $(this).data("method") ? $(this).data("method") : "POST"
            );
            if ($(this).data("submit")) {
                $(
                    "#processingModal #processingModalForm #processingModalSubmit"
                ).attr("type", "submit");
            } else {
                $(
                    "#processingModal #processingModalForm #processingModalSubmit"
            ...
            $("#processingModal #processingModalTitle").html(
                $(this).data("title")
            );
            $("#processingModal #processingModalBody").html(
                $(this).data("body")
            );
            $(
                "#processingModal #processingModalForm #processingModalSubmit"
            ).html($(this).data("submit-title"));
            $(
                "#processingModal #processingModalForm #processingModalCancel"
            ).html($(this).data("cancel-title"));
        }
    });
            ...
            if (parent == showId) {
                ...
                if (
                    ...
                    (Array.isArray(dataShow) &&
                        (dataShow.includes(showValue) ||
                            dataShow.includes(parseInt(showValue))))
                ) {
                    $(this).addClass("show");
                } else {
                ...
                if (dataHide !== undefined) {
                    if (
                        showValue == dataHide ||
                        (Array.isArray(dataHide) &&
                            (dataHide.includes(showValue) ||
                                dataHide.includes(parseInt(showValue))))
                    ) {
                        $(this).removeClass("show");
                    } else {
                        $(this).addClass("show");
    ...
    $(".sync-selected, .connect-selected").click(function (e) {
        ...
        if (url) {
            ...
            $("#invoicesTable td.checkbox .checkInput input").each(function () {
                if (this.checked) {
                    checked = true;
                    $("#form-update-selected-content").append(
                        $(this).clone().removeAttr("id").hide()
                    );
                }
            });
            if (checked) {
    ...
    $(".add-user-setting-relationship").submit(function (e) {
        ...
        let relationshipModel = $(this)
            .find("#relationship_collection_model")
            .val();
                ...
                if (res.status === 200) {
                    ...
                    if (relationshipTable) {
                        let html =
                            ...
                            "</td><td>" +
                            data.right_title +
                            '</td><td><a href="javascrript:;" class="remove-user-setting-relationship" data-href="/remove-user-setting-relationship?relationship_collection_model=' +
                            relationshipModel +
                            "&do_not_unique=" +
                            doNotUnique +
                            "&left_val=" +
    ...
    $(".inventory-location")
        ...
        .change((e) => {
            ...
            let id = Object.values(locations).find(
                (id) => parseInt(id) === parseInt(e.target.value)
            );
            if (id !== undefined) {
                let location = $(e.target).children(":selected").text();
                alert(
                    `${location} already selected. Please choose another location.`
                );
                e.target.value = "";
        ...
        ele.submitDataAjax({
            ...
            success: function success(res) {
                if (res.status === 200) {
                    ...
                    const nextElement = $(
                        `[data-group=${hsGroup}][data-level=${nextLevel}]`
                    );
    ...
    $(".add-user-setting-mapping").submit(function (e) {
        ...
        var leftTitle =
            $("#" + mappingKey + "left_value option:selected").html() ||
            leftValue;
        ...
        if (group) {
            ...
            groupInputs.each(function () {
                ...
                if (item.val()) {
                    rightTitle[itemLevel] =
                        item.find("option:selected").html() || item.val();
                } else {
                    rightTitle[itemLevel] = "";
                }
            });
            ...
            rightDefaultValue =
                rightInput.find("option:first-child").val() || "";
            ...
            if (rightValue) {
                if ($.isArray(rightValue)) {
                    if ($.inArray("all", rightValue) != -1) {
                        ...
                        if (
                            $("#" + mappingKey + "right_value option:selected")
                                .length > 1
                        ) {
                            var rightTitle = "";
                            $(
        ...
        if (
            !leftValue ||
            (leftInput[0].tagName.toLowerCase() === "select" &&
                leftValue === "0")
        ) {
            return;
        }
        ...
        $(this).submitDataAjax({
            ...
            success: function (res) {
                ...
                if (group) {
                    $(".setting-mapping-group-input")
                        .val(rightDefaultValue)
                        .trigger("change");
                } else {
                ...
                if (res.status === 200) {
                    if (isDelete) {
                        table
                            .find(
                                "tr.mapping-row[data-mapping_key='" +
                                mappingKey +
                                "'][data-left_value='" +
                                leftValue +
                                "']"
    ...
    $(".setting-mapping-group-input").change(function (event) {
        ...
        var nextInput = table.find(
            `.setting-mapping-group-input[data-level=${nextLevel}]`
        );
        if (nextInput.length) {
            nextInput.submitDataAjax({
                ...
                data: {
                    level: nextLevel,
                    parent_value: ele.val()
                },
                success: function (result) {
                    if (result.status === 200) {
                        let options = "";
                        for (var i in result.results) {
                            options += `<option value="${i == 0 ? "" : i}">${result.results[i]
                                }</option>`;
                        }
    ...
    $(".nav.nav-tabs li").on("click", function () {
        ...
        $(this).closest(".container-fluid").find(".nav-tab-content > .tab-pane").removeClass("active");
        $(this).closest(".container-fluid").find(".nav-tab-content").find($(this).find('a').attr("href")).addClass("active");
    });
        ...
        $("#fileUploadModalForm").ajaxSubmit({
            ...
            success: function (res) {
                if (successMessage) {
                    ...
                    alert(
                        "Existing products have been connected successfully."
                    );
                }
                window.location.reload();
            },
    ...
    $(".datatable-filter:not([type=radio]), .datatable-filter__select-change").on("change", datatableChange);
    ...
    if($("select.bs-multiselect").length)
    {
    ...
    $("#selectDateModalSubmit").click(function () {
        var validDate = validateDate(
            document.getElementById("selectDateModalInput").value
        );
        if (!validDate) {
        ...
        var successMessage = $(
            "#selectDateModal #selectDateModalMessage"
        ).html();
        waitingDialog.show("Please wait...");
    ...
    $(".setting-mapping-one-one").on(
        "change",
        ".setting-mapping-one-one-input",
        function (event) {
            ...
            if (rightValue) {
                ...
                container
                    .find(
                        `.setting-mapping-one-one-input[data-left-value!=${leftValue}]`
                    )
                    .each(function () {
...
function appendSettingMapping($table, $data, $group) {
    ...
    if (exists.length) {
        ...
        var tr = $(
            '<tr class="mapping-row" data-mapping_key="' +
            $data.mapping_key +
            '" data-left_value="' +
            $data.left_value +
            '"></tr>"'
        ...
        tr.append(
            '<td><a href="javascript:void(0);" onclick="removeSettingMapping(this)">Remove</a></td>'
        );
...
function buildNSCustomFieldInputValue($data) {
    var row =
        $(`<div class="row dependency-div show netsuite-custom-field-value" data-parent="${$data.group}" data-show="1">
         <div class="col-md-12">
...
function applyNSCustomFieldInputValue($data) {
    ...
    if (input.length) {
        ...
        var container = $(
            '.netsuite-custom-field-container[data-group="' + $data.group + '"]'
        );
...
function buildSettingMenu() {
    ...
    for (var key in settingMenus) {
        if (counter % numberMenu === 0) {
            item = $(
                '<li class="carousel-item" style="width:100%"><ul data-target="#setting-menu" data-slide-to="0"></ul></li>'
            );
        }
        var menu = settingMenus[key];
        var nav = $(
            '<li class="' + (menu.current ? "current" : "") + '"></li>'
        );
        var link;
        if (menu.current) {
        ...
        if (menu.disabled) {
            link = $(
                '<a href="javascript:;" data-toggle="modal" data-target="#generalNotActive" title="' +
                menu.title +
                '">' +
                menu.label +
                "</a>"
        ...
        counter++;
        if (
            (counter !== 0 && counter % numberMenu === 0) ||
                ...
                success: function (res) {
                    if (res.status === 200) {
                        ...
                        if (res.ids != "undefined" && res.ids) {
                            $(
                                "#invoicesTable td.checkbox .checkInput input"
                            ).each(function () {
                                let id = $(this).val();
...
$(document).on("click", "button[data-toggle], a[data-toggle]", function(e) {
    e.preventDefault();
    ...
    if(type=="popup")
    {
        ...
        href = $(this).data('href');
        target = $(this).data('target')==undefined?$("#modalFormUpdate"):$($(this).data('target'));
        target.modal("show");
...
$(document).ready(function(){
    if( $("#alert_message").length && $("#alert_message").text().trim() != "")
    {
        showNoti($("#alert_message").text(),"Thông báo","ok");
    }
    if( $("#alert_message_error").length && $("#alert_message_error").text().trim()  != "")
    {
        showNoti($("#alert_message_error").text(),"Thông báo","error");
    }
});

function checkUpdateFrm() {
    var form = $('#updateFrm');
    return checkValidate(form);
}
function checkValidate(form)
{
    ...
    form.find('[data-required="1"]').each(function () {
        ...
        var trEle = inputEle.closest("div");
        ...
        if (inputEle.attr('type') == 'checkbox') {
            var name = inputEle.attr('name').replace('[]', '');
            if ($("input[name^='" + name + "']:checked").length == 0) {
                var id = inputEle.attr('id');
                var msg = "Vui lòng chọn ít nhất mốt sự lựa chọn";
        ...
        // if ((value == '' || value == null) && type == undefined && !trEle.hasClass('hide') && inputEle.find("option:selected").attr("disabled") == undefined) {
        if(value == '' || value == null){
            err = 1;
            tag_top = $(this).parent().offset().top - 50;
            if (tag_top > 600) {
                $('html, body').animate({
                    scrollTop: parseInt(tag_top)
                }, 1000);
            }
            ...
            if (!validatePhone(value)) {
                err = 1;
                create_error($("#"+id), id);
                trEle.find('.errordiv').text("Số điện thoại thì không chính xác");
                showErrOfField(id, id);
                return false;
            }
            // return true;
            ...
            if (!validateEmail(value)) {
                err = 1;
                create_error($("#"+id), id);
                trEle.find('.errordiv').text("Email thì không chính xác");
                showErrOfField(id, id);
                return false;
            }
        ...
        else if(type == "editor")
        {
            if(strip_tags(value)=="")
            {
                err = 1;
                create_error($("#"+id), id);
                trEle.find('.errordiv').text("Chưa nhập nội dung");
                showErrOfField(id, id);
                return false;
            }
        }
...
function create_error(ele, id)
{
    if($(".errordiv."+id).length == 0)
    {
        text = $("#"+id).attr("placeholder")!=undefined?$("#"+id).attr("placeholder"):$("#"+id).attr("title");
        bindHtml = `<div class="errordiv ${id}"><div class="arrow"></div>${text!="" && text!=undefined?text:lang("Please input data")}!</div>`;
        $( bindHtml ).insertAfter( ele);
    }
...
function showNoti(mes, title, mode = 'ok', type="") {

    if(type=="text")
    {
        $("#alert_message").html(mes);
    }
    else if(type == "toast")
    {
        showToast(mes);
        ...
        var icon = 'ok';
        if (mode == 'Err' || mode == 'err' || mode == 'error') {
            mode = 'Err';
            icon = 'fa fa-info remove';
        } else if (mode == 'War' || mode == 'war' || mode == 'warning') {
            mode = "War";
            icon = 'fa fa-info';
        }
        else{
...
/* Modal Popup */
$(document).on("click",".closeModal, button.close, .btn-modal-close",function(e){
    e.preventDefault();
...
$(document).on('click','.action-delete',function(){
    href = $(this).data('href');
    ids = $("input[name='ids[]']:checked").map(function () {
        return this.value;
    }).get();
    ...
    $.confirm({
        title: 'Xác nhận!',
        content: 'Bạn có muốn xóa các dữ liệu được chọn!',
        buttons: {
            confirm: {
                text: 'Đồng ý',
                btnClass: 'btn-blue',
                ...
                action: function(){
                    ...
                    $.ajax({
                        ...
                        $.LoadingOverlay("hide");
                        showNoti(res.message,lang("Notification"),"warning")
                        setTimeout(function () {
                            location.reload();
                        }, 1000);
                    });
...
$(document).on('change','.check-all',function(){
    ...
    let forEle = $(this).attr("for");
    if(forEle!= undefined)
    {
        $(`input[name*=${forEle}]`).prop("checked",checked);
    }
...
var lang_json = {
    "vi":{
        "Please input data":"Vui lòng nhập dữ liệu",
        "Warning" : 'Cảnh báo',
...
function lang(msg)
{
    return lang_json[default_lang][msg];
}
    ...
    $("body").keydown(function(e){
        ...
        if(keyCode==112)
        {
            ...
            if(page!="" && page != undefined)
            {
                $.ajax({
                    method: "POST",
                    url: base_url +  "/admin/modules/guide",
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
                    data: {
                    ...
                    if($("#helper").length==0)
                    {
                        helper = `<div class="modal fade" id="helper">
                    ...
                    $("#helper").find(".modal-body").html(res);
                    $("#helper").modal("show");
                });
            }
        }
        ...
        if(keyCode==121)
        {
            ...
            if(page!="" && page != undefined)
            {
                $.ajax({
                    method: "POST",
                    url: base_url +  "/admin/modules/info",
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
                    data: {
                    ...
                    href = base_url +  "/admin/modules/update/"+res.id;

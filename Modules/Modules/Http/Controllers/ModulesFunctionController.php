...
use Modules\Modules\Model\ModuleFunction;
use Modules\Plugin\Entities\Themes;
use App\Model\base_model;
class ModulesFunctionController extends BaseController
{
    ...
    public function __construct()
    {
        ...
        $this->module = Modules::where("file","modules_function")->first();
    ...
    function index(Request $request)
    {
        ...
        $data = array(
            ...
            'url_update' => url('admin/modules_function/update'),
            'rows' => ModuleFunction::get(),
        ...
        return Themes::render('modules::modules_function.index', $data);

    }
    public function update($id = null)
    {
        $data = array(
            'info' => ModuleFunction::where("id",$id)->first(),
            'rights' => !empty($info['rights']) ? json_decode($info['rights']) : [],
            // 'updated' => $this->updated,
            // 'name' => $this->name,
            'fields' => $this->fn->module_fields($GLOBALS['var']['act']),
            'action' => site_url($GLOBALS['var']['act'] . '/process') . $this->uri_str,
        );
        return  Themes::render('modules::modules_function.update', $data);
    }

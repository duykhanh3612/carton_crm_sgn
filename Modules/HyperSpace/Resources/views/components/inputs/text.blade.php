@php
    ...
    $type = (isset($type) && in_array($type, ['text', 'number', 'date', 'password', 'file', 'hidden' , 'email','search'])) ? $type : 'text';
    ...
    <div class="col-md-{{$colLeft}}">
        <div class="form-group">
            ...
            <input
                ...
                type="{{ $type }}" @if(!empty($required)) required @endif @if(!empty($placeholder)) placeholder="{{ $placeholder }}" @endif
                name="{{ $name }}" @if(!empty($dataType)) data-type="{{ $dataType }}" @endif
                value="{{ $value ?? '' }}" @if(!empty($dataValue)) data-value="{{ $dataValue }}" @endif
                id="{{ $name }}"
                @isset($min)
                min="{{ $min }}"
                @endif

...
@endphp
@if(!h::isMobile())
<div class="col-md-6 form-group desktop" title="join_list">
    <div class="col-md-4 filter-input-group">
        <label class="filter-input-control">{!!$ctrl->title !!}</label>
    </div>    
    <div class="col-md-8">        
        <?=h::select("src[".$ctrl->att_key_join."][like][none]", @$src['none'],@$ctrl->att_style.' class="src_'.$ctrl->name.' form-control filter-input-control"',
             @$table,'1=1 '.($ctrl->att_where==''?'':' and '.$ctrl->att_where),$field_lang,@$ctrl->att_key,@$ctrl->att_orderby,

@php
   ...
   // $field_lang = @$ctrl->att_field_language==1?$ctrl->att_field.$lang:$ctrl->att_field;
    $field_lang = @$ctrl->language==1?$ctrl->att_field.$lang:$ctrl->att_field;
@endphp
<div class="form-group {{$ctrl->width}} " data-title="treelistlor">
    <label>
        <?=@$ctrl->title?>
    </label>
    <?=\h::tree($ctrl->name,@$row->{$ctrl->value},$ctrl->att_style.' class="form-control listbox '.$table.'" ',

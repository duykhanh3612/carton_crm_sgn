...
$funs = App\Model\md::find_all("sys_function","","group_id,ordering,orderby");
$group = @h::array_group($funs->toArray(),'group_id',true);

$list_per = json_decode(@$row->{$ctrl->value.$lang},true);
$per = @h::array_group(@$list_per,'function_id',true);

@endphp
<div class="form-group {{  $ctrl->width}} desktop " title="role">
...
@foreach($group as $g=>$g_v)
@php
$function = App\Model\md::find_all("sys_function","group_id='".$g."'","group_id,ordering,orderby");
@endphp
<label class="title" style="border-top:1px solid #eee">{{ $g }}</label>
<hr style="boder-top:1px solid" />
@foreach($function as $func)
<div class="col-lg-12 item_fuction" style="border-bottom:dashed 1px;">
    <div class="col-md-6">
        <div class="form-group">

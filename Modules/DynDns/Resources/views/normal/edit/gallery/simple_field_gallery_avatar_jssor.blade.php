...
<div class="form-group {{@$ctrl->width}}" id="gallery_{{$ctrl->name}}">
    ...
    <div class="portlet-content" style="width:100% !important;">
        ...
        <input type="hidden" id="<?=@$ctrl->att_where?>" name="<?=@$ctrl->att_where?>" value="{{@$row->{@$ctrl->att_where} }}" />
            ...
            <div class="list_item">
                ...
                <div class="row list_item_row" style="padding:10px; border-bottom:1px dashed #bd2742">
                    <div class="form-group col-md-1">
                        ...
                        <input type="radio" name="avatar" {{ @$d->avatar==true?'checked':''}} class="row_avatar" value="{{@$d->image}}"  /> Ảnh đại diện
                        ...
                        <div class="col-md-{{@$col}} ctxdform image">
                            ...
                            <div class="col-md-12 lable_upload text-center">
                                ...
                                <div id="{{$ctrl->name}}thumb_upload{{@$item_image}}" class="thumb-output">
                                    @if(@$d->image!='')
                                    <a class="thumb_image" data-toggle="modal" data-target="#myModalImage">
                                        <img src="{{    url($base.@$d->image)}}" />
                                    </a>
                                    <script type="text/javascript">
                                        image_string += '<div>' + 
                                                          '<img data-u="image" src="{{    url($base.@$d->image)}}" />' +
                                                          '<img data-u="thumb" src="{{    url($base.@$d->image)}}" />' +
                                                          '<span class="caption">{{    @$d->title}} - {{    @$d->content}}</span>' +
                                                      '</div>';
                                    </script>
                                    @else
                    ...
                    @if(@$ctrl->read!=1 && !$flg_read)
                    <label class="btn btn-sm warning row_remove hidden-sm-up pointer"  data-id="m_confirm_gallery_delete{{$ctrl->name}}">
                        <small>
                            Xóa
                        </small>
                    </label>
            ...
            <div class="{{$ctrl->name.@$lang}}_list_item_default" style="display:none">
                <div class="row list_item_row" style="padding:10px; border-bottom:1px dashed #bd2742">
                    <div class="form-group col-md-1" style="padding:9px;">
                        ...
                        <input type="radio" name="avatar" class="row_avatar" value="image"  /> Chọn làm ảnh đại diện
...
<script>
    ...
    $(document).on('change', '.content_{{$ctrl->value }} .row_avatar', function () {
        var image = $(this).parent().parent().find('.row_image').val();
        $(e_avartar).val(image);
    ...
    $(document).on('click', '.btn-gallery-item', function () {
        var img = item_chose.parent().parent().find('.row_image').val();
        if (img == '') {
            ...
            $.get("{{url('admin/'.request()->segment(2).'/remove_upload')}}", { image: "public/qtland/" + img }, function (data) {
                item_chose.parent().parent().remove();
                {{ $ctrl->value }}_update_galler_content();
                $(popup_id).modal('hide');
            });
    ...
    $(document).on('keyup', '.content_{{ $ctrl->value }} .item', function () {
            {{ $ctrl->value }}_update_galler_content();
    });
        ...
        var file_data = $(this).prop("files")[0];
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if ($(input_file).parent().parent().parent().parent().find('.row_avatar').prop("checked") == true ||
                        $(input_file).parent().parent().parent().parent().parent().find('.list_item_row').length == 1) {

                        $('#<?=$ctrl->att_where?>').val(image);
                        $(input_file).parent().parent().parent().parent().find('.row_avatar').prop("checked", 'true');
                }
    ...
    function {{ $ctrl->value }}_update_galler_content() {
        ...
        $('.content_{{$ctrl->value }} .list_item>.list_item_row').each(function () {
            ...
            if (avatar == true)  $('#<?=$ctrl->att_where?>').val(image);

@php
if(App\Model\Admin::groupID()!='1' && App\Model\Admin::groupID()!='2'  && $ctrl->name!='photos')
    $flg_read =    true;
else $flg_read = false;
@endphp
<div class="form-group {{@$ctrl->width}}" id="gallery_{{$ctrl->name}}" data-title="simple_field_gallery_avatar">
    <?php
    if($ctrl->att_table!='')
    ...
    <div class="portlet-content" style="width:100% !important;">
        ...
        @push("popup")
        <div id="m_confirm_gallery_delete{{$ctrl->name}}" class="modal" >
        ...
        <div class="content_{{ $ctrl->value }}">
            ...
            <div class="list_item">
                ...
                <div class="row list_item_row" style="padding:10px; border-bottom:1px dashed #bd2742">
                    ...
                    <div class="form-group col-md-1">
                        ...
                        <input type="radio" name="avatar" {{ @$d->avatar==true?'checked':''}} class="row_avatar" value="{{@$d->image}}"  /> Chọn làm ảnh đại diện
                    ...
                    <div class="form-group col-md-1">
                        @if(@$ctrl->read!=1 && !$flg_read)
                        <label class="row_remove  d-none d-sm-block" data-id="m_confirm_gallery_delete{{$ctrl->name}}">
                            <i class="fa fa-remove" style="color:red"></i> Xóa
                        </label>
                        @endif
                    </div>
                    @if(@$ctrl->read!=1 && !$flg_read)
                    <label class="col-md-12 row_remove  d-block d-sm-none"  data-id="m_confirm_gallery_delete{{$ctrl->name}}">
                        Xóa
                        <b>{{    @$d->title}}</b>
                    </label>
                    @endif
            ...
            <div colspan="5" style="padding-top:5px;">
                <label class="btn btn-fw primary {{@$ctrl->read==1?" cccccc":"item_add"}}" style="cursor:pointer;">
                    <i class="fa fa-plus" style="color: #ff6a00"></i>
                    &nbsp; Thêm mới
                </label>
            </div>
            @endif
...
<script>
    ...
    $(document).on('click', '.btn-gallery-item', function () {
        item_chose.parents('.list_item_row').remove();
        {{ $ctrl->value }}_update_galler_content();
        $(popup_id).modal('hide');
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if($(input_file).parent().parent().parent().parent().find('.row_avatar').prop("checked")==true)
                        $(e_avartar).val(data.image);
                    $(input_file).parent().find('.row_image').val(data.image);
                    {{$ctrl->value }}_update_galler_content();

...
$field_value = @$row->{$ctrl->value.@$lang};
if(is_array($field_value)){
    $field_value = json_encode($field_value);
}
@endphp
<div class="form-group {{@$ctrl->width}} grid_gallery_panel panel_{{ $ctrl->value }}" id="gallery_{{$ctrl->name}}" data-title="simple_field_grid_gallery_base_path">
    <?php
    if(@$ctrl->att_table!='')
    ...
    <div class="portlet-content" style="width:100% !important;">
        ...
        <textarea id="{{$ctrl->name}}" name="{{$ctrl->name.@$lang}}" class="form-control content_id" style="display:none">{{ $field_value }}</textarea>
            ...
            <div class="list_item row">
                @php
                    ...
                    if(!is_array($field_value)){
                        ...
                        $results_json = convertObject($field_value);
                    }
                    $item_image = 0;
                @endphp
...
<script>
    ...
    var item_chose;
    var popup_id;
    $(document).on('click', '.content_{{ $ctrl->value }} .row_remove', function () {
        item_chose = $(this)
    ...
    //Upload multi
    $(document).on('change', '.content_{{$ctrl->value }} .gallery_upload_multiple', function (e) {
        var id = $('#id').val();
        var tag = $(this);
        if (id == "") {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if ($('#id').val() == '')
                        $('#id').val(data.id);
                    upload_multiple(tag,"{{ $ctrl->value }}_update_galler_content","{{ $ctrl->value }}");
                }
            });

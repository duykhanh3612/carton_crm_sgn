<div class="form-group {{@$ctrl->width}}" >
    ...
    <div class="<?=@$row->{$ctrl->value.@$lang}?>">
        <textarea id="{{$ctrl->name}}" name="{{$ctrl->name}}" class="form-control content_id" style="display:none">
            <?=@$row->{$ctrl->value.@$lang}?>
        </textarea>
        ...
        <table class="table table-striped  b-t">
            ...
            <tbody class="list_item">
                @php
                    $results_json = json_decode(@$row->{$ctrl->value});
                    $item_image = 0;
                @endphp
...
<script>
    ...
    $(document).on('click', '.item_add', function () {
        ...
        html_ele.find('.row_no').val(count_image-1);  

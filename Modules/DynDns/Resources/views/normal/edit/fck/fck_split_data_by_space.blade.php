        ...
        <script>
            ...
            $('#{{@$ctrl->name.$lang}}').change(function () {
                ...
                for (i = 0; i < data.length; i++) {
                    ...
                    {
                        if (field[i] == 'area' || field[i] == 'home_wide_road' || field[i] == 'area_wide' || field[i] =='area_long')
                             $('#'+field[i]).val(Number(data[i]));
                        else{
                            if(data[i]!='')
                            {
                                var str_value = data[i];

                                if (str_value[0] == '"' && str_value[str_value.length - 1] == '"')
                                   str_value = str_value.substr(1, str_value.length - 2);
                                
                                if(field[i]!=''){
                                    $('#' + field[i]).val(str_value);

                                    var value = str_value.replace(new RegExp(',', 'g'), '').trim();
                                    $('#' + field[i] + '_hide').val(value);
                                }

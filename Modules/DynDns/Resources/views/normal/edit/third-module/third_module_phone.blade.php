...
<div class="form-group {{  $ctrl->width}} desktop ">
    ...
    <div class="">
        <input type="text" class="form-control {{@$ctrl->validate==1?'validation':''}}  {{ @$ctrl->needed==1?'needed':''}}   <?=$lang?>" 
               ...
               id="<?=@$ctrl->name.$lang?>"   value="<?=@$tr->{$ctrl->value.$lang}?>" placeholder=" {{        $ctrl->title }} " data-callback="replace_space_phone<?=@$ctrl->name.$lang?>" />
        ...
        <script>
            ...
            function replace_space_phone<?=@$ctrl->name.$lang?>() {
                $('#<?=@$ctrl->name.$lang?>').val($('#<?=@$ctrl->name.$lang?>').val().replace(/\s/g, ''));
            }
        </script>
    </div>
</div>

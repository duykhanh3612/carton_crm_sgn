...
<div class="row mb-2">
    <div class="col-sm-12">
        <select name="type" id="type_input" class="form-control" data-type="{{ @$row->type }}" data-value="{{ $jsonValue }}">
            ...
            <option {{ @$row->type == 'text.textarea' ? 'selected' : '' }} value="text.textarea" data-mask="textarea">Văn bản dài</option>
    ...
    <div class="col-sm-12 box-field-type" data-name="{{ $name }}">
        @if (!empty(@$row->type))
            ...
            @if (view()->exists("admin::components.input.$type"))
                {!! load_view("components.input.$type", ['name' => $name, 'value' => $value, 'id' => $id, 'mask' => @$mask],"atc") !!}
            @else
                {!! load_view('components.input.text', ['name' => $name, 'value' => $value, 'id' => $id, 'mask' => @$mask],"atc") !!}
            @endif
        @endif
    </div>
</div>
@push('js')
    <script>
        ...
        var site_url = window.location.origin +"/atc/";

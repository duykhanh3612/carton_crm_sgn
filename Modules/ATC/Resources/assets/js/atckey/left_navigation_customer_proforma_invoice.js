...
LeftNavigation.prototype.send = function (data, url, option = {}) {
    ...
    return new Promise((resolve, reject) => {
        $.ajax({
            url: 'customer_proforma_invoice/' + url,
            type: "POST",
            data: data,
            beforeSend: function () {
...
LeftNavigation.prototype.updateLeftNavigation = async function () {
    ...
    this.root.find("[data-group]").each(function () {
        ...
        if(remove==true){
            updatemenuparentleft(1,sort_order,group_id,name,'customer_proforma_invoice_navigation');
        }
        if (remove == false) {
            updatemenuparentleft(0,sort_order,group_id,name,'customer_proforma_invoice_navigation');
        }
        ...
        $(this).find(".item").each(function () {
            ...
            if(del==true){
                updatemenuleft(1,sort_order,id,group_id,item_name,badge,url,'customer_proforma_invoice_navigation');
            }
            if (del == false) {
                updatemenuleft(0,sort_order,id,group_id,item_name,badge,url,'customer_proforma_invoice_navigation');
            }

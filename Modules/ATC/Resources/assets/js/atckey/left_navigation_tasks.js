...
LeftNavigation.prototype.addEventListener = function () {
  ...
  $("body")
    .on("click", "#fake-close-sidebar", function () {
...
LeftNavigation.prototype.addNewItem = function () {
  ...
  if (type == "Group") {
    this.root.find(".wapper-category")
      .append(`<div data-new="1" class="group-category" data-group="${group_id}">
        <div class="filter">
...
LeftNavigation.prototype.setLeftHeaderFixedTable = function (
  el,
  animation = false
) {
  /*   var cookie_closed = $.cookie('closed-sidebar-task');
...
LeftNavigation.prototype.updateLeftNavigation = async function () {
  ...
  this.root.find("[data-group]").each(function () {
    ...
    if (remove == true) {
      updatemenuparentleft(
        1,
        sort_order,
        group_id,
        name,
        "tasks_navigation_new"
      );
    }
    if (remove == false) {
      updatemenuparentleft(

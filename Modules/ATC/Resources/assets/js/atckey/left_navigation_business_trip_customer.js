...
LeftNavigation.prototype.send = function (data, url, option = {}) {
    ...
    return new Promise((resolve, reject) => {
        $.ajax({
            url: 'business_trip_customer/' + url,
            type: "POST",
            data: data,
            beforeSend: function () {
...
LeftNavigation.prototype.saveIndexLeftNavigation = function(el){
    ...
    var index = $(el).attr("data-item");
    console.log({index});
    $.cookie("btc_leftnav",index);
}
LeftNavigation.prototype.setActiveLeftNavigation = function(){
    var index = $.cookie("btc_leftnav");
    console.log({index})
    if(index){
...
LeftNavigation.prototype.toggleSidebar = function () {
    ...
    $.cookie('closed-sidebar-btc', $('body').hasClass('closed-sidebar') ? 1 : 0, {
        path: '/'
    });
...
LeftNavigation.prototype.visibleSidebar = function () {
    var cookie_closed = $.cookie('closed-sidebar-btc');
    if (cookie_closed != "1") {

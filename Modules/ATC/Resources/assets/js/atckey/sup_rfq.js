...
$(document).ready(function ($) {
    ...
    $('body').on('click', '.btn-sale-rfq', function () {
        ...
        var approvedCheck = tr.find('#approvedCheck').val();
        ...
        $.ajax({
            ...
            type: 'POST',
            cache: false,
            data: {part: part,approvedCheck:approvedCheck},
            success: function (string) {
       ...
       check_attribution_mfr_sup();
        ...
        parent.find('.Totalweight').val(Totalweight);
        total_tfoot();
    ...
    function execute_cost(costID) {
        ...
        $('.select-supplier option[value="' + costID + '"]:selected').each(function () {
            buyingAmountTotal += parseFloat($(this).closest('tr').prev('tr').find('.buying-amount').val().replace(/\s/g, '').replace(/,/g, ''));
        })
        ...
        var TotalCOP1234 = 0;
        $('.select-supplier option[value="' + costID + '"]:selected').each(function () {
            orderQuantity12 = parseFloat($(this).closest('tr').prev('tr').find('.order-qty').val().replace(/\s/g, '').replace(/,/g, ''));
            costofgoodssold12 = parseFloat($(this).closest('tr').prev('tr').find('.costofgoodssold').val().replace(/\s/g, '').replace(/,/g, ''));
    ...
    function check_attribution_mfr_sup() {
        var arr = [];
        var arrCostID = [];
        var arrCostTotal = [];
        var arrCostPercent = [];
       ...
       if (arrCostID.length) {
        ...
        if ($('#attribution tbody tr').length) {
            $('#attribution tbody tr').each(function() {
                ...
                arrMethodRFQ[$(this).data('id')] = $(this).find('.SMethodRFQ').find('select').html();
        ...
        if (arrCostID.length) {
            for (var i = 0; i < arrCostID.length; i++) {
              ...
              //  alert(arrCostID[i]);
                ...
                html += '   </td>';
              //  html += '   <td><div class="input-group"><input type="hidden" name="attrCost[' + arrCostID[i] + '][ShippingMethodRFQ]" class="form-control attr-Weight" value="' + (arrMethodRFQ[arrCostID[i]] > 0 ? arrMethodRFQ[arrCostID[i]] : 0)  + '"> </div></td>';
               if(arrMethodRFQ[arrCostID[i]]){
                html += ' <td class="SMethodRFQ"><select name="attrCost[' + arrCostID[i] + '][ShippingMethodRFQ]" id="ShippingMethodRFQ'+arrCostID[i]+'" class="form-control select2"> '+arrMethodRFQ[arrCostID[i]]+'</select></div></td>';

             //   html += ' <td class="SMethodRFQ">'+arrMethodRFQ[arrCostID[i]]+'</div></td>';
              }else{
                html += ' <td class="SMethodRFQ"><select name="attrCost[' + arrCostID[i] + '][ShippingMethodRFQ]" id="ShippingMethodRFQ'+arrCostID[i]+'" class="form-control select2"> '+suppliers_options+'</select></div></td>';
              }
           ...
           //    $('#ShippingMethodRFQ').trigger( 'chosen:updated' );
        ...
        $('.SMethodRFQ').find('.select2').chosen();

    //    $('#ShippingMethodRFQ'+arrCostID[i]+'').chosen();;
    
       // $('#ShippingMethodRFQ'+arrCostID[i]+'').val(suppliers_options).trigger( 'chosen:updated' );

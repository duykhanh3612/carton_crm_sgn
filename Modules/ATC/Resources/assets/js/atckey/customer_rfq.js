$(document).ready(function ($) {
    ...
    $('body').on('click', '.btn-sale-rfq', function () {
        ...
        var trpre = $(this).closest('tr').prev();
        trpre.find('.up-so').val(updates + 1);
        var key = trpre.attr('id');
        trpre.find('input[name^=products], textarea[name^=products], select[name^=products]').each(function () {
        ...
        $.ajax({
            url: site_url + 'customer_rfq/update_sale_rfq',
            type: 'POST',
            data: {
                rfq_id: rfq_id,
            ...
            success: function (string) {
                if (string == 1) {
                    ...
                    showNoti('Supplier Part #: ' + trpre.find('.supplier-part').val(), 'Sales Order update successful', 'Ok');
                    if (updates == 0) {
        ...
        $.ajax({
            url: site_url + 'sup_rfq/update_cus_rfq',
            type: 'POST',
            data: {
                rfq_id: rfq_id,
                salesorder_id: salesorder_id,
        ...
        var selling_price_vnd = tr.find('.selling_price_vnd');
        var selling_amount = tr.find('.selling-amount');
        var selling_amount_vnd = tr.find('.selling_amount_vnd');
        var LineCardID = tr.find('.LineCardID');
        var CustomerTargetPrice = tr.find('.CustomerTargetPrice');
        var trnext = $(this).closest('tr').next();
        

        var QuoteStaff = trnext.find('.QuoteStaff');
        var QuoteID = trnext.find('.QuoteID');
        var Incoterm = trnext.find('.Incoterm');
        var SupplierLeadTime = trnext.find('.SupplierLeadTime');
        var CustomerLeadTime = trnext.find('.CustomerLeadTime');
        var Firmware = trnext.find('.Firmware');
        var BDC = trnext.find('.BDC');
        var COO = trnext.find('.COO');
        var StartDate = trnext.find('.StartDate');
        var ValidityNum = trnext.find('.ValidityNum');
        var ValidityDate = trnext.find('.ValidityDate');
        var ActiveStatus = trnext.find('.ActiveStatus');
        ...
        $.ajax({
            ...
            success: function (string) {
                ...
                $('.get-info-item').click(function () {
                    ...
                    var did = tr_item.find('.SupplierRfqdid').val();
                    $.ajax({
                        url: site_url + 'customer_rfq/get_info_supplier_rfq',
                        type: 'POST',
                        cache: false,
                        data: {
                            id: did,
                            table: 'supplier_rfq_details',
                            // act: 'business_trip_customer'
                        },
                        success: function (string) {
                            var getData = $.parseJSON(string);
                            QuoteStaff.val(getData.QuoteStaff).trigger('chosen:updated');  
                            QuoteID.val(getData.QuoteID).change();   
                            Incoterm.val(getData.Incoterm).change();   
                            SupplierLeadTime.val(getData.SupplierLeadTime).change();   
                            CustomerLeadTime.val(getData.CustomerLeadTime).change();   
                            Firmware.val(getData.Firmware).change();   
                            BDC.val(getData.BDC).change();   
                            COO.val(getData.COO).change();  
                            StartDate.val(getData.StartDate).change();   
                            ValidityNum.val(getData.ValidityNumber).change();   
                            ValidityDate.val(getData.ValidityDate).change();   
                            ActiveStatus.val(getData.ActiveStatus).trigger('chosen:updated');   
                            LineCardID.val(getData.Supplier).trigger('chosen:updated');   
                            CustomerTargetPrice.val(getData.CustomerTargetPrice).change();
                            showNoti('Supplier Part Number: ' + part, 'Cập nhật hoàn tất', 'Ok');
                        }
                    })
                    var sellingprice = tr_item.find('.SellingPrice-old').text().replace(/\s/g, '').replace(/,/g, '');
                    var orderqty = tr_item.find('.order-quantity-old').text().replace(/\s/g, '').replace(/,/g, '');
                    var sellingamount = sellingprice * orderqty;
                    var usd_rates = $('#usd_rates').text().replace(/\s/g, '').replace(/,/g, '');
                    var price_vnd =  (usd_rates*sellingprice);
                    var price_vnd_amout = price_vnd * orderqty ;

                    selling_price_vnd.val(accounting.formatMoney(price_vnd),'',0).change(); 
                    selling_amount.val(accounting.formatMoney(sellingamount),'',4).change(); 
                    selling_amount_vnd.val(accounting.formatMoney(price_vnd_amout),'',0).change(); 
                    ...
                    selling_price.val(accounting.formatMoney(parseFloat(tr_item.find('.SellingPrice-old').text().replace(/\s/g, '').replace(/,/g, '')), '', 4)).change();
        ...
        var sellingPrice = parseFloat(parent.find('.selling-price').val().replace(/\s/g, '').replace(/,/g, ''));
        ...
        // }else {
            parent.find('.selling_amount_vnd').val(accounting.formatMoney(buyingPrice, '', 2));
            parent.find('.selling_price_vnd').val(accounting.formatMoney(orderQuantity * buyingPrice, '', 2));
        // }
    ...
    }).on('change', '.UnitWeight', function () {
        var parent = $(this).closest('tr');
        var parentprev = $(this).closest('tr').prev();
        var orderQuantity = parseFloat(parentprev.find('.order-qty').val().replace(/\s/g, '').replace(/,/g, ''));
        var UnitWeight = parseFloat(parent.find('.UnitWeight').val().replace(/\s/g, '').replace(/,/g, ''));

        var Totalweight = UnitWeight * orderQuantity;
        parent.find('.Totalweight').val(accounting.formatMoney(Totalweight, '', 4));
        total_tfoot();

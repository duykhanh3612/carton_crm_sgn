...
LeftNavigation.prototype.toggleSidebar = function () {
    ...
    $.cookie('closed-sidebar-rp', $('body').hasClass('closed-sidebar') ? 1 : 0, {
        path: '/'
    });
...
LeftNavigation.prototype.visibleSidebar = function () {
    var cookie_closed = $.cookie('closed-sidebar-rp');
    if (cookie_closed != "1") {
    ...
    $.each(items, function (k, v) {
        var item = Sortable.create(v, {
            ...
            onEnd: function (/**Event*/evt) {
                var type_move = _this.isDifferentGroup(evt)
                // disabled drag filter
                if (evt.newIndex == 0 && type_move) {
                    var item = evt.item;
                    evt.item.remove();
                    $(evt.from).append(item);
                }
                if(evt.newIndex == 0 && type_move == false){
                    var filter = $(evt.to).find(".filter");
                    $(evt.to).find(".filter").remove();
                    $(evt.to).prepend(filter);
                }
                _this.updateLeftNavigation();
            }
        })
...
LeftNavigation.prototype.updateLeftNavigation = async function () {
    ...
    this.root.find("[data-group]").each(function () {
        ...
        if(remove==true){
            updatemenuparentleft(1,sort_order,group_id,name,'request_pickup_navigation');
        }
        if (remove == false) {
            updatemenuparentleft(0,sort_order,group_id,name,'request_pickup_navigation');
        }
        ...
        $(this).find(".item").each(function () {
            ...
            if(del==true){
                updatemenuleft(1,sort_order,id,group_id,item_name,badge,url,'request_pickup_navigation');
            }
            if (del == false) {
                updatemenuleft(0,sort_order,id,group_id,item_name,badge,url,'request_pickup_navigation');
            }

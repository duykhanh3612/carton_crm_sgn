...
Approved.prototype.removeChosen = function(){
    ...
    setTimeout(()=>{
        try {
            ...
            console.log("chosen unavailable ")
        }
        ...
        try{
            ...
            console.log("moment unavailable")
        }
...
Approved.prototype.select2 = async function(){
    ...
    this.root.find("#ap-information").find('.select2').select2({
        placeholder: 'Select an option'
      });
    var len = this.root.find("#list_approved").find('.select2').length;

    if(len){
        
        this.root.find("#list_approved").find(".select2").each(function(){
        $(this).select2({
            placeholder: "Select an option",
          });
      })
...
Approved.prototype.approved = function(el){
        ...
        $.alerts.confirm1('Are you sure you confirm this item? Successfully confirming the system will lock the system cannot be adjusted. Please double check before doing, Thanks !!!<br />', 'Confirm ', function(r) {
            ...
            if(type){
                app.send({id,type,cscid},'changeStatus');
            }
            setTimeout(function() {

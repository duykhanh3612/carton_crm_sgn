...
LeftNavigation.prototype.send = function (data, url, option = {}) {
    ...
    return new Promise((resolve, reject) => {
        $.ajax({
            url: 'customer_sales_contract/' + url,
            type: "POST",
            data: data,
            beforeSend: function () {
...
LeftNavigation.prototype.toggleSidebar = function () {
    ...
    $.cookie('closed-sidebar-csc', $('body').hasClass('closed-sidebar') ? 1 : 0, {
        path: '/'
    });
    this.visibleSidebar();
...
LeftNavigation.prototype.visibleSidebar = function () {
    var cookie_closed = $.cookie('closed-sidebar-csc');
    if (cookie_closed != "1") {
...
LeftNavigation.prototype.updateLeftNavigation = async function () {
    ...
    this.root.find("[data-group]").each(function () {
        ...
        if(remove==true){
            updatemenuparentleft(1,sort_order,group_id,name,'customer_sales_contract_navigation');
        }
        if (remove == false) {
            updatemenuparentleft(0,sort_order,group_id,name,'customer_sales_contract_navigation');
        }
        ...
        $(this).find(".item").each(function () {
            ...
            if(del==true){
                updatemenuleft(1,sort_order,id,group_id,item_name,badge,url,'customer_sales_contract_navigation');
            }
            if (del == false) {
                updatemenuleft(0,sort_order,id,group_id,item_name,badge,url,'customer_sales_contract_navigation');
            }
...
LeftNavigation.prototype.updateBadges = async function () {
    ...
    var module = $("#act").val();
    if($.cookie('csc_tabs') =='detail'){
        module = module + '_views';
    }

    var res = await this.send({ data,module }, "getBadges", { showProcess : false });

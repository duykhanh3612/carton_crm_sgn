...
Stock_import.prototype.unformatCurrency =  function(el){
...
Stock_import.prototype.unformatCurrencyValue = function(value){
...
Stock_import.prototype.removePoDuplicate = function(po){
    var count_po = 0;
...
Stock_import.prototype.clockRemovePo = function(){
    if(this.id){
        $("#mainTable-stock_import").find('.fg-sc select').addClass("");
        $("#mainTable-stock_import").find('.fg-sc input').addClass("");
        // $("#mainTable-request_pickup").find(".btn-remove-po").addClass("disabled");
        $("#mainTable-stock_import").find(".fg-sc").last().find(".btn-remove-po").removeClass("");
        
    }
}
Stock_import.prototype.addChosen = function(el=null){
...
Stock_import.prototype.send = function(data,url,option={}){
    // var domain = '/'+window.location.pathname +"request_pickup"
    return new Promise((resolve, reject) => {
        $.ajax({
            url:'stock_import/'+ url,
            type: "POST",
            data: data,
            beforeSend:function(){
                showProcess(1);
                Stock_import.addLoading();
            },
            complete:function(){
                hideLoading();
                Stock_import.removeLoading();
            },
...
Stock_import.prototype.refreshTableItem = async function(){
...
class MainTable extends Stock_import{
    constructor(){
...
MainTable.prototype.createInsertPartTable = function(data,stt){
    ...
    data.forEach(function(item){
        ...
        body+='<tr>';
        body+='<td>'+MainTable.BtnCheckPart(item['SCID'],item['did'],i)+'</td>';
...
MainTable.prototype.createInsertPartTablecpo = function(data){
    ...
    data.forEach(function(item){
      //  console.log(item);
       ...
       body+='<td style="width: 100px; min-width: 100px; max-width: 100px;" class="Manufacturer" id="Manufacturer">'+item['Manufacturer']+'</td>';
...
MainTable.prototype.changeSelectSC = async function(el){
    ...
    if(res) form_groups.find("[name*=EndCustomerName]").val(res.EndCustomerName.replace('["','').replace('"]','').replace('"',''));
    if(res) form_groups.find("[name*=import_method]").val(res.ImportMethod.replace('["','').replace('"]','').replace('"',''));
...
class ItemList extends Stock_import {
    constructor(el=null){
...
class ModalListOld extends Stock_import {
    constructor(){
...
class ModalListOldcpo extends Stock_import {
    constructor(){

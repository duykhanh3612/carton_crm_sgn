...
LeftNavigation.prototype.toggleSidebar = function (el) {
    $("body").toggleClass("closed-sidebar")
    $.cookie('closed-sidebar-task', $('body').hasClass('closed-sidebar') ? 1 : 0, {
        path: '/'
    });
...
LeftNavigation.prototype.visibleSidebar = function () {
    var cookie_closed = $.cookie('closed-sidebar-task');
    if (cookie_closed != "1") {
...
LeftNavigation.prototype.setLeftHeaderFixedTable = function (el, animation = false) {
   var cookie_closed = $.cookie('closed-sidebar-task');
    ...
    setTimeout(()=>{
        $('#mainTable-tasks').stickyTableHeaders({
            scrollableArea: $(".main_container")
        });
    },500)
...
LeftNavigation.prototype.setSortable = function (mode = "create") {
    ...
    $.each(items, function (k, v) {
        var item = Sortable.create(v, {
            revert: true, // Enable swap plugin
            group: 'shared',
            handle: '.item',
            swapClass: 'highlight', // The class applied to the hovered swap item
            animation: 150,
...
LeftNavigation.prototype.updateLeftNavigation = async function () {
    ...
    this.root.find("[data-group]").each(function () {
        ...
        var module=$('#act').val();
        if($('#act').val()=='projects_customer_part') module='projects_customer';

        remove = remove ? remove : false;
        var name = $(this).find(".filter span").eq(0).text();
        if(remove==true){
            updatemenuparentleft(1,sort_order,group_id,name,module+'_navigation');
        }
        if (remove == false) {
            updatemenuparentleft(0,sort_order,group_id,name,module+'_navigation');
        }
        ...
        $(this).find(".item").each(function () {
            ...
            if(del==true){
                updatemenuleft(1,sort_order,id,group_id,item_name,badge,url,module+'_navigation');
            }
            if (del == false) {
                updatemenuleft(0,sort_order,id,group_id,item_name,badge,url,module+'_navigation');
            }
...
LeftNavigation.prototype.updateBadges = async function () {
    ...
    var data = [];
    var _this = this;
   // if($('#act').val()=='projects_customer_part') module='projects_customer_part';
    ...
    var res = await this.send({ data }, "getBadges", { showProcess : false });

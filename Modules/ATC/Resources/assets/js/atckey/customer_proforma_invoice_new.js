class Customer_proforma_invoice_new {
    ...
    afterLoaded() {
        Customer_proforma_invoice_new.removeLoading();
    }
    addEventListener() {
        var _this = this;
...
Customer_proforma_invoice_new.prototype.submitPartDisplayOptions = async function(){
...
Customer_proforma_invoice_new.prototype.partDisplayOptions = async function(){
    var module = $('#act').val();
...
Customer_proforma_invoice_new.prototype.createDragTasks = function (name) {
    name = name + '-body';
...
Customer_proforma_invoice_new.prototype.exchangeDeposit = function(el){
...
Customer_proforma_invoice_new.prototype.removeDeposit = function(el){
...
Customer_proforma_invoice_new.prototype.updateExchange = function(){
    var url = window.location.href
    url = url.replace("?update_usd=1","");
    window.location.href=url+'?update_usd=1';
}
Customer_proforma_invoice_new.prototype.changeCheckBox = function(el){
    var isChecked = $(el).prop('checked');
    var value = isChecked ? 1 : 0;
    $(el).val(value);
}
Customer_proforma_invoice_new.prototype.changeCSC = async function (el) {
    var value = $(el).val();
...
Customer_proforma_invoice_new.prototype.getSelectInvoiceHtml = function (data, index){
    var option = '';
...
Customer_proforma_invoice_new.prototype.changeCPO = async function(el){
    var id = $(el).val();
...
Customer_proforma_invoice_new.prototype.removeCPO = function(el){
...
Customer_proforma_invoice_new.prototype.addDeposit = async function () {
...
Customer_proforma_invoice_new.prototype.addCPo = async function (el) {
...
Customer_proforma_invoice_new.prototype.checkCarry = function(){
...
Customer_proforma_invoice_new.prototype.unformatCurrency = function (el) {
...
Customer_proforma_invoice_new.prototype.unformatCurrencyValue = function (value) {

    value = value ? accounting.unformat(value) : 0
    // value = value.replace(/[0-9].-/g,'')
    return value;
}
Customer_proforma_invoice_new.prototype.formatWeight = function (el, symbol = 'kgs', normal = true) {
...
Customer_proforma_invoice_new.prototype.formatWeightValue = function (value, symbol = 'kgs', normal = true) {
...
Customer_proforma_invoice_new.prototype.formatCurrency = function (el, isUnit = false, currency = 'USD') {
...
Customer_proforma_invoice_new.prototype.formatCurrencyValue = function (value, isUnit = false, currency = 'USD') {
    var isInt = round(value) == value;
...
Customer_proforma_invoice_new.prototype.scrollTopSmooth = function (el) {
    // off/on for test
...
Customer_proforma_invoice_new.prototype.removePoDuplicate = function (po) {
    var count_po = 0;
...
Customer_proforma_invoice_new.prototype.addChosen = function (el = null) {
...
Customer_proforma_invoice_new.prototype.addDatePicker = function (key,group) {
    key = key +1;
    // add date picker
...
Customer_proforma_invoice_new.prototype.send = function (data, url, option = {}) {
    url = option.ajaxController == true ? url : 'customer_proforma_invoice_new/' + url;
    return new Promise((resolve, reject) => {
        $.ajax({
            ...
            beforeSend: function () {
                showProcess(1);
                Customer_proforma_invoice_new.addLoading();
            },
            complete: function () {
                hideLoading();
                Customer_proforma_invoice_new.removeLoading();
...
Customer_proforma_invoice_new.prototype.addRemoveListItems = function () {
    var rp = this;
...
Customer_proforma_invoice_new.prototype.setStateExecuteBtn = function (mode = "turn") {
    if (mode == "turn") {
...
Customer_proforma_invoice_new.prototype.cloneBtnUpdate = function (mode = "edit") {
...
Customer_proforma_invoice_new.prototype.disabledBtnUpdate = function () {
    if ($("#OrderType").val() == 0) {
        // alert edit
        showNoti('Please press the Execute cost button before pressing the UPDATE button', "Warning edit data", "War");
        // set state execute
...
Customer_proforma_invoice_new.prototype.triggerRefreshValuePo = function (itemList, table) {
...
Customer_proforma_invoice_new.prototype.refreshTableItem = async function () {
...
class MainTable extends Customer_proforma_invoice_new {
    constructor() {

        super("#mainTable-customer_proforma_invoice_new")
...
class ModalListOld extends Customer_proforma_invoice_new {
    constructor() {
...
class ItemList extends Customer_proforma_invoice_new {
    constructor(el = null) {
...
class ExportLog extends Customer_proforma_invoice_new {
...
$(document).ready(function () {
    var crp = new Customer_proforma_invoice_new();

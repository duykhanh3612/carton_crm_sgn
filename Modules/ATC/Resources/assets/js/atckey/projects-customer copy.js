...
// if ($('#Shipmentsm'+keypar+'').length) {
  //  key = parseInt($('.colactivedetail'+keypar+':last .key').val()) + 1;
  //  numb = parseInt($('.colactivedetail'+keypar+':last .numb').val()) + 1;
...
if ($('.PotentialLineCardquote'+parentid+'').length) {
    keyac =$(this).data('key');
    key = parseInt($('.PotentialLineCardquote'+parentid+':last .key').val()) + 1;
// numb = parseInt($('.PotentialLineCardquote:last .numb').text()) + 1;
}else{
    keyac =$(this).data('key');
...
$('#itemList table tbody tr.tr-last-'+keypar+'').before(string);
setTimeout(function(){
autoComplete();

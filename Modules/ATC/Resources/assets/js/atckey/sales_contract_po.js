...
sales_contract.prototype.unformatCurrency =  function(el){
...
sales_contract.prototype.unformatCurrencyValue = function(value){
...
sales_contract.prototype.removePoDuplicate = function(po){
    var count_po = 0;
...
sales_contract.prototype.clockRemovePo = function(){
    if(this.id){
        $("#mainTable-stock_import").find('.fg-po select').addClass("");
        $("#mainTable-stock_import").find('.fg-po input').addClass("");
        // $("#mainTable-request_pickup").find(".btn-remove-po").addClass("disabled");
        $("#mainTable-stock_import").find(".fg-po").last().find(".btn-remove-po").removeClass("");
        
    }
}
sales_contract.prototype.addChosen = function(el=null){
...
sales_contract.prototype.send = function(data,url,option={}){
    ...
    return new Promise((resolve, reject) => {
        $.ajax({
            ...
            beforeSend:function(){
                showProcess(1);
                sales_contract.addLoading();
            },
            complete:function(){
                hideLoading();
                sales_contract.removeLoading();
            },
...
sales_contract.prototype.refreshTableItem = async function(){
...
class MainTable extends sales_contract{
    constructor(){
...
MainTable.prototype.insertPart = async function(el){
    ...
    var value =form_groups.find("input.PoCode").val();
    //alert(value);
    // form_groups.find("input .PoCode").val();
    var key1 = form_groups.find("input.stt").val();
  //  alert(key1);
//
...
//
  ...
  //  var data = ['PODate','ContractNo','ContractDate','CustomerPONo','CPODate','ImportMethod'];
    ...
    //if(had_po==0){
        ...
        var data  = await this.send({value},'getListPartcpo');
              //console.log(data);
...
MainTable.prototype.createPoRow = function(options,key){
    ...
    return `<div id= 'fg-po' class="form-group group-required fg-po fg-po-${key}" data-selected="${key}"> <div class="">
                    <div class="col-sm-12">
                        <div class="">
                        ...
                        <div class="col-sm-3">
                           ${options.sc}
                            ...
                            <input type="hidden" class="PoCode"  id="PoCode${key}" value="">
                            <input type="hidden" class="stt"  id="stt${key}" value="${key}">

                            <div class="col-sm-2">
                            <input type="text" class="form-control" name="PODate[]" id="PODate${key}" value="" readonly >
                        </div>
                       
                       
                        
                    <div class="col-sm-1">
                    <input type="text" class="form-control col-approve" id="approve${key}" name=""   value="" readonly data-required="1">
                    <div class="errordiv Shipping_Cost${key}">Not Empty</div>
                
                </div>
                        <div class="col-sm-1">

                            <input type="text" class=" form-control col-back_cost" name="" id="POStatus${key}" value=""  readonly>
                            <div class="errordiv POStatus${key}">Not Empty</div>
                        
                        </div>

                    <div class="col-sm-1">
                    <input type="text" class="form-control col-approve" id="ImportMethod${key}" name="ImportMethod[]"   value="" readonly >
                    <div class="errordiv Shipping_Cost${key}">Not Empty</div>
                
                
                    </div>
                    <div class="col-sm-2">
                        <input type="text" class="form-control" name="" id="EndCustomerName${key}" value="" readonly >
                    </div>
...
MainTable.prototype.addPo = async function(el){
    var group_insert = $("#sc-information").find(".fg-po[data-selected]");
  //  alert(group_insert.length);
       ...
       // options['sc'] = await this.send({key:key,supplier:supplier },"htmlSelectSC");
        ...
        options['sc'] = await this.send({key},"htmlSelectPo");
...
MainTable.prototype.addDatePicker = function(key){
    // add date picker
    $('#sc-information .fg-po-' + key).find('.bootstrap-datepicker').datepicker({
        format: 'yyyy-mm-dd',
        language: 'vi',
        autoclose: true,
        todayHighlight: true
...
MainTable.prototype.createInsertPartTable = function(data,stt){
    ...
    <table class="table-modal table table-hover">
        ...
        <tr class="nodrop">
            ...
            <th  style="width: 100px; min-width: 100px; max-width: 100px;">PO </th>
            <th  style="width: 100px; min-width: 100px; max-width: 100px;">CPO</th>
            <th  style="width: 100px; min-width: 100px; max-width: 100px;">	Import Method</th>

            <th  style="width: 150px; min-width: 100px; max-width: 100px;">Supplier Part </th>
            <th  style="width: 200px; min-width: 200px; max-width: 200px;">Mfr Part </th>
            <th  style="width: 100px; min-width: 100px; max-width: 100px;">Manufacturer</th>
            <th  style="width: 100px; min-width: 100px; max-width: 100px;">	Package / Case</th>
            <th  style="width: 100px; min-width: 100px; max-width: 100px;">	Packaging</th>

            <th  style="width: 100px; min-width: 100px; max-width: 100px;">SPQ</th>
            <th  style="width: 100px; min-width: 100px; max-width: 100px;">Order Qty </th>
            <th  style="width: 100px; min-width: 100px; max-width: 100px;">Unit price (USD)</th>
            <th  style="width: 100px; min-width: 200px; max-width: 200px;">Leadtime</th>
    ...
    data.forEach(function(item){
        ...
        body+='<tr class="poold">';
        body+='<td  style=" width: 30px; ">'+MainTable.BtnCheckPart(item['id'],item['CPOID'],i)+'</td>';
        body+='<td style=" display: none; "  class="OrderQuantity" id="po'+i+'">'+item['POID']+'</td>';
        body+='<td style=" display: none; " class="OrderQuantity" id="CPOID'+i+'">'+item['CPOID']+'</td>';
        body+='<td   class="OrderQuantity">'+item['POID']+' - '+item['code']+'</td>';

        body+='<td class="OrderQuantity">'+item['CPOID']+' - '+item['codecpo']+'</td>';
        body+='<td class="ImportMethod">'+item['ImportMethod']+'</td>';
        
        body+='<td class="supplier_part" id= "supplier_part'+i+'">'+escapeHtml(item['SupplierPart'])+'</td>';
        body+='<td class="MfrPart"><span id="MfrPart'+i+'">'+escapeHtml(item['MfrPart'])+'</span><span class="desc" id="desc'+i+'">'+item['Description']+'</span></td>';
        ...
        body+='<td class="StandardPackageQty" id="StandardPackageQty'+i+'">'+item['StandardPackageQty']+'</td>';
        body+='<td class="OrderQuantity" id="OrderQuantity'+i+'">'+item['OrderQuantity']+'</td>';

        body+='<td id="UnitPriceUSD'+i+'">'+item['UnitPriceUSD']+'</td>';
        body+='<td id="LeadtimeComments'+i+'">'+item['LeadtimeComments']+'</td>';

        body+='<td style=" display: none; " id="key'+i+'">'+i+'</td>';
        body+='<td style=" display: none; " class="stt" id="stt'+i+'">'+i+'</td>';
        body+='<td style=" display: none; " id="ImportMethod1'+i+'">'+item['ImportMethod']+'</td>';

       // delete item['SCID'];
       // delete item['did'];

       /* body+='<td class="supplier_part" id= "supplier_part'+i+'">'+escapeHtml(item['SupplierPart'])+'</td>';
        body+='<td class="MfrPart"><span id="MfrPart'+i+'">'+escapeHtml(item['MfrPart'])+'</span><span class="desc">'+item['Description']+'</span></td>';
        body+='<td class="Manufacturer" id="Manufacturer'+i+'">'+item['Manufacturer']+'</td>';
        body+='<td class="qty" id="qty'+i+'">'+item['OrderQuantity']+'</td>';
        body+='<td class="priceusd" id="priceusd'+i+'">'+item['UnitPriceUSD']+'</td>';
        body+='<td style=" display: none; " id="SCID'+i+'">'+item['SCID']+'</td>';
...
MainTable.prototype.removePo = function(el){
    ...
    var po = $(el).closest(".fg-po").find(".ContractNo option:selected").text();
    var poid = $(el).closest(".fg-po").find(".SCCode").val();
...
MainTable.prototype.changeSelectSC = async function(el){
   ...
   // var html = await this.createInsertPartTable(data);
    ...
    //this.openModelInsertPart(html);
    var lengthsc=  $('#sclist .fg-po').length;
  //alert(lengthsc);
   // var key = ($(el).data('key'));
   var ContractNo = '';
   var arr = [];
...
$('.scchange'+changsc+'').remove();
    ...
    //  $("#cpolistsc"+value+"").remove();  
     ...
     // $("#scpolist").prepend('<div class="fg-posc" id="cpolistsc'+key+'"></div>');
...
MainTable.prototype.changeSelectPo = async function(el){
    var value = $(el).val();
       // alert(value);
       var group_insert = $(el).data('key');
       //alert(group_insert);
       changepo(group_insert,value)
...
class ItemList extends sales_contract {
    constructor(el=null){
...
class ModalListOld extends sales_contract {
    constructor(){
...
class ModalListOldcpo extends sales_contract {
    constructor(){

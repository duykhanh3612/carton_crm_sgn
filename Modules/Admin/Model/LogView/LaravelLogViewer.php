...
namespace Modules\Admin\Model\LogView;
 ...
 */
class LaravelLogViewer extends Model
{
    /**
     * @var string file
     */
    ...
    {
        $this->level = new LogLevel();
        $this->pattern = new LogPattern();
        $this->storage_path = base_path("storage/logs"); // function_exists('config') ? config('logviewer.storage_path', storage_path('logs')) : storage_path('logs');
    ...
    public function getLogs()
    {
        ...
        if (strpos($this->file, 'client') !== false) {
            $this->get_log_hyper($log, $file);
        } else {
            $this->get_log($log, $file);
        }
    ...
    function get_log_hyper(&$log, $file)
    {
        preg_match_all($this->pattern->getPattern('logs_hyper'), $file, $headings);

        if (!is_array($headings)) {
            return $log;
        }

        $log_data = preg_split($this->pattern->getPattern('logs_hyper'), $file);
        if ($log_data[0] < 1) {
            array_shift($log_data);
        }
        $lines = explode("\n", $file);
        foreach ($headings[0] as $index => $h) {
            $level = "info";
            $line = array_search($headings[0][$index], $lines) + 1;
            $content = $headings[4][$index];

            $content = explode(" ", $content);
            $class = [];
            foreach ($content as $text) {
                preg_match_all('/.*(App(.*))/i', $text, $controller);
                if (current($controller[0]) != "" && \Str::startsWith(current($controller[0]), "App"))
                    $class[] = current($controller[0]);
            }
            if (\Str::contains($headings[0][$index], ['fail', 'missing', 'error'])) {
                $level = "error";
            }
            $log[] = array(
                'context' => 'Line ' . $line, // $headings[1][$index],
                'level' => $level,
                'class' => $class,
                'folder' => $this->folder,
                'level_class' => $this->level->cssClass($level),
                'level_img' => $this->level->img($level),
                'date' => $headings[3][$index], // date('Y-m-d h:i:s',strtotime($headings[3][$index])),
                'text' => $headings[4][$index],
                'in_file' => "",
                'stack' => $headings[0][$index],
                'line' => $line
            );
        }
        return $log;
    }
    function get_log(&$log, $file)
    {
        ...
        foreach ($headings as $h) {
            for ($i = 0, $j = count($h); $i < $j; $i++) {
                foreach ($this->level->getAll() as $level) {

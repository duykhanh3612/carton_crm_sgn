...
<div class="row {{ $rowClass ?? '' }}" @if (!empty($rowData)) @foreach ($rowData as $dataKey=> $dataVal)
    ...
    <div class="col-md-{{ $colLeft }} thumb-img">
        <div class="form-group {{ @$ctrl->width }} grid_gallery_panel panel_{{ $name }}" id="gallery_{{ $name }}" data-title="simple_field_grid_gallery_base_path">
            @if (!empty($label))
                ...
                <div class="content_{{ $name }}">
                    <div class="list_item row">
                        ...
                        <div class="  col-md-2 list_item_row" style="padding:10px;">
                            <input class="pull-left form-control row_order item" {{ @$ctrl->read == 1 || $flg_read ? 'readonly' : '' }} type="text"
                            value="{{ @$d->no }}" />

                            <div class="container_avatar">
                                <label class="container">
                                    <input type="radio" name="{{ $name }}_avatar" {{ @$d->avatar == true ? 'checked' : '' }} class="row_avatar"
                                    value="{{ @$d->image }}"
...
@push('js')
<link href="{{ asset('') }}public/plugin/lightbox2/dist/css/lightbox.css" rel="stylesheet" />
<script src="{{ asset('') }}public/plugin/lightbox2/dist/js/lightbox.js"></script>
<script>
            ...
            for (var i = 0; i < files.length; i++) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        //image
                        html_ele.find('.btn_upload').attr('for', '{{ $name }}_upload' + count_image +
                            1);
                        html_ele.find('.lable_upload>.add-file-upload').attr('id',
                            '{{ $name }}_upload' + count_image + 1);

@php
    $reference_id = @$id;
    $reference_type = $module->file;
    $slug = @$id ==""?null:md::find("slugs","reference_id='$reference_id' and reference_type='$reference_type'");
@endphp
<div class="pre">
    ...
    <script>
        $('#name').on('change', function () {
               $.get("{{ url(request()->segment(1).'/'.request()->segment(2).'/slug_alias') }}",{'title':$(this).val(),'lang':'','id':$("#id").val()},function(msg){
                   $('#permalink').val(msg);
               });
        ...
        if ($('#name').val() != "") {
            $.get("{{ url(request()->segment(1).'/'.request()->segment(2).'/slug_alias') }}",{'title':$('#name').val(),'lang':'','id':$("#id").val()},function(msg){
                $('#permalink').val(msg);
            });
        }
        ...
        $("#permalink_create_slug").click(function(){
            let value = $('#name').val();
            $.get("{{ url(request()->segment(1).'/'.request()->segment(2).'/slug_alias') }}",{'title':value,'lang':'','id':$("#id").val()},function(msg){
                   $('#permalink').val(msg);
            });
        });
    </script>
